...
| Decision module | online | Threshold (initial **0.7**) on sigmoid |
...
```mermaid
flowchart TD
    A[Live source\n(file or webcam)] --> B[Frame preprocessor\nCLAHE? -> Gaussian 3x3 -> 640x640 -> BGR->RGB]
    B --> C[YOLOv8n-Pose\n17 COCO keypoints]
    C --> D[Keypoint filter\nconf < 0.5 unreliable]
    ...
    I --> J[Sigmoid score p]
    J --> K{p >= 0.7?}
    K -- yes --> L[Decision: Violence]
    K -- no  --> M[Decision: NonViolence]
    L --> N[Visualizer overlay]
    M --> N
    H -->|buffer not full| O[No decision (warm-up)]
```
...
Applies the **initial threshold 0.7** to the sigmoid output. Threshold may be retuned per use case (see `evaluation.md`).
...
- Whether the online pipeline performs any temporal smoothing on consecutive decisions: **TBD.**
