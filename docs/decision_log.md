...
| D21 | Online decision zones         | **Triple-zone**: p < 0.35 → NonViolence (green); 0.35 ≤ p < 0.45 → Suspicious (yellow); p ≥ 0.45 → Violence (red) | Binary threshold only                                                               | Suspicious zone provides early warning without triggering a full alert; reduces false alarms from borderline cases                                                                      | Implemented in src/inference.py                           |
| D22 | Temporal smoothing            | **3-window majority vote**: last 3 decisions kept; min 2 must be Violence/Suspicious to trigger alert             | No smoothing; single-frame decision                                                 | Suppresses transient single-frame noise; window of 3 gives 1-2 frame lag which is acceptable at 10 FPS                                                                                  | Implemented in src/inference.py                           |
...
- D21 — Triple-zone decision (NonViolence / Suspicious / Violence)
- D22 — Temporal smoothing: 3-window, min 2 count
...
- **Deployment decision threshold** — initial **0.7** → RLVS-only model tuned to **0.4** (val F1=0.773, test F1=0.827, AUC=0.921) → blended model (RLVS+RWF-2000) re-tuned to **0.45** (test F1=0.820, AUC=0.927). See D25.
...
- **Temporal smoothing of consecutive online decisions**: Resolved — 3-window majority vote, min 2 count. See D22.
...
- Whether multi-source training data is in scope for v2: Resolved — RWF-2000 integrated in v2. See D24.
